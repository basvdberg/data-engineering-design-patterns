...
CLI driver. Run from the `full-data-solution/` directory:
...
python -m adl.Extractors.wfs --mapping knmi-daggegevens-temperature
```

## Configuration

The extractor is driven by a DWA-style JSON mapping stored at
`adl/DataObjectMappings/knmi-daggegevens.json`. Key extensions on the mapping:
...
3. Create a mapping JSON in `adl/DataObjectMappings/` with `gml_parser` set to
   `<name>`.
