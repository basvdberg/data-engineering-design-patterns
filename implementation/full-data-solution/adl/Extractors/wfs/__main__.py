...
Run as a module from the ``full-data-solution/`` directory::

    python -m adl.Extractors.wfs --list
    python -m adl.Extractors.wfs --mapping knmi-daggegevens-temperature --info
    python -m adl.Extractors.wfs --mapping knmi-daggegevens-temperature --page-size 2 --max-features 4 --limit 500
"""
...
PACKAGE_DIR = Path(__file__).resolve().parent               # adl/Extractors/wfs/
EXTRACTORS_ROOT = PACKAGE_DIR.parent                        # adl/Extractors/
ADL_ROOT = EXTRACTORS_ROOT.parent                           # adl/
SOLUTION_ROOT = ADL_ROOT.parent                             # full-data-solution/

if str(SOLUTION_ROOT) not in sys.path:
    sys.path.insert(0, str(SOLUTION_ROOT))

from adl.Extractors.common import config as cfg_module
from adl.Extractors.common import parquet as parquet_module
from adl.Extractors.wfs import client as wfs_client
from adl.Extractors.wfs import gml_parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--config",
        default=str(ADL_ROOT / "DataObjectMappings" / "knmi-daggegevens.json"),
